...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!json.success) { setError(json.error || 'Registration failed'); setLoading(false); return; }
    ...
    <div className="min-h-dvh pitch-bg flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-1 text-sm">Join ScoreXI and start scoring</p>
        </div>
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} className="input-field" placeholder="Your name" required minLength={2} maxLength={80} />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="input-field pr-10" placeholder="Min 8 characters" required minLength={8} />
          ...
          <p className="text-center text-sm text-slate-400 mt-5">
            ...
            <Link href="/auth/login" className="text-brand-400 hover:text-brand-300 font-semibold">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
