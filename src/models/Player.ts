...
const playerSchema = new Schema<PlayerDoc>({
  ...
  // FIX: unique:true on field already creates an index — do NOT also call schema.index({ username:1 })
  username:   { type: String, trim: true, lowercase: true, maxlength: 30, sparse: true, unique: true },
  phone:      { type: String, trim: true, sparse: true },
  email:      { type: String, trim: true, lowercase: true, sparse: true },
  // FIX: userId indexed ONLY via schema.index() below — no index:true on the field
...
// Each index declared exactly ONCE
// username: unique:true above already creates the index — no duplicate needed
playerSchema.index({ name: 'text' });
playerSchema.index({ userId: 1 });   // only here, not on the field definition
